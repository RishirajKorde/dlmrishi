...
const Sidebar = ({ isCollapsed, setCollapsed, isMobile, isOpen, onClose }) => {
  ...
  const getMenuItems = () => {
    ...
    if (role === 'SUPERADMIN' || role === 'SUPER_ADMIN' || role === 'LIBRARIAN' || role === 'LIBRARIAN_ADMIN') {
      return [
        ...baseItems,
        { to: '/members', icon: Users, label: 'Members' },
        { to: '/transactions', icon: ArrowLeftRight, label: 'Transactions' },
        { to: '/fines', icon: Receipt, label: 'Fines' },
        { to: '/search', icon: ShieldCheck, label: 'search' },
  ...
  const getMasterItems = () => {
    ...
    return [
      ...
      { to: '/Language', icon: ShieldCheck, label: ' Language' },
